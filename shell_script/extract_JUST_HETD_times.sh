...
grep "(9-4-3) Device to Host Data Transfer Time:" "$INPUT_FILE" | awk '{print $(NF-1)}'

echo "Only Heterogeneous kernel start ~ sync start:"
grep "(9-4-4) Only Heterogeneous kernel start ~ sync start:" "$INPUT_FILE" | awk '{print $(NF-1)}'

echo "Host To Device Data Transfer Time:"
grep "(9-4-5) Host To Device Data Transfer Time:" "$INPUT_FILE" | awk '{print $(NF-1)}'
