...
KMCUDAResult kmeans_cuda(
  ...
  //INFO("reassignments threshold: %" PRIu32 "\n", uint32_t(tolerance * samples_size)); // 10,000
  ...
  size_t centroids_size = static_cast<size_t>(clusters_size) * features_size; // 1024 * 128
  FOR_EACH_DEV(
    if (dev == device_ptrs) {
  ...
  CUMALLOC(device_assignments_prev, samples_size);  // host -> device
  ...
  // record kmeans_yy_cuda start time
  ...
  // record kmeans_yy_cuda end time
  cudaEventRecord(stop);
  cudaEventSynchronize(stop);
  
  float milliseconds = 0;
  cudaEventElapsedTime(&milliseconds, start, stop);
  printf("Yinyang K-Means total execution time: %f ms\n", milliseconds);
//----------------------------------------------------------------------------------------------------------------------------------
  ...
  if (origin_devi < 0) {
    if (device_ptrs < 0) {
      CUCH(cudaMemcpy(centroids, device_centroids[devs.size() - 1].get(),
                      centroids_size * sizeof(float), cudaMemcpyDeviceToHost), kmcudaMemoryCopyError);
      CUCH(cudaMemcpy(assignments, device_assignments[devs.size() - 1].get(),
                      samples_size * sizeof(uint32_t), cudaMemcpyDeviceToHost), kmcudaMemoryCopyError);
    } else {
      CUCH(cudaMemcpyPeerAsync(centroids, device_ptrs,
                               device_centroids[devs.size() - 1].get(),
                               devs.back(), centroids_size * sizeof(float)), kmcudaMemoryCopyError);
      CUCH(cudaMemcpyPeerAsync(assignments, device_ptrs,
                               device_assignments[devs.size() - 1].get(),
                               devs.back(), samples_size * sizeof(uint32_t)), kmcudaMemoryCopyError);
      SYNC_ALL_DEVS;
    }
  }
